...
async function extractProvenance(workspace, rootDir, dir, fileName, provenance = {}) {
    ...
    provenance = {
        ...provenance,
        [relativeTargetFileName]: (0, requirements_file_parser_1.parseRequirementsFile)(requirementsTxt),
    };
    const { containsRequire, matches } = await (0, contains_require_directive_1.containsRequireDirective)(requirementsTxt);
    if (containsRequire) {
        for (const match of matches) {
            ...
            if (Object.keys(fixedCache).includes(pathLib.normalize(pathLib.join(dir, base)))) {
                handlerResult.succeeded.push({
                    ...
                    changes: [
                        {
                            success: true,
                            userMessage: `Fixed through ${(0, format_display_name_1.formatDisplayName)(entity.workspace.path, {
...
async function fixIndividualRequirementsTxt(workspace, dir, entryFileName, fileName, remediation, parsedRequirements, options, directUpgradesOnly) {
    ...
    const { updatedManifest, changes } = (0, update_dependencies_1.updateDependencies)(parsedRequirements, remediation.pin, directUpgradesOnly, entryFilePath !== fullFilePath
        ? (0, format_display_name_1.formatDisplayName)(workspace.path, {
            type: 'pip',
...
async function applyAllFixes(entity, options) {
    const { remediation, targetFile: entryFileName, workspace, } = (0, validate_required_data_1.validateRequiredData)(entity);
    const fixedMeta = {};
    const { dir, base } = pathLib.parse(entryFileName);
    const provenance = await (0, extract_version_provenance_1.extractProvenance)(workspace, dir, dir, base);
    ...
    const { changes: pinnedChanges } = await fixIndividualRequirementsTxt(workspace, dir, base, fileForPinning.fileName, toPin, (0, requirements_file_parser_1.parseRequirementsFile)(fileForPinning.fileContent), options, directUpgradesOnly);
...
function filterOutAppliedUpgrades(remediation, upgradeChanges) {
    ...
    const normalizedAppliedRemediation = upgradeChanges
        ...
        if (c.success && c.from) {
            ...
            return `${(0, standardize_package_name_1.standardizePackageName)(pkgName)}@${versionAndMore}`;
        }
        return false;
    })
        .filter(Boolean);
    for (const pkgAtVersion of Object.keys(pins)) {
        ...
        if (!normalizedAppliedRemediation.includes(`${(0, standardize_package_name_1.standardizePackageName)(pkgName)}@${versionAndMore}`)) {
...
function calculateRelevantFixes(requirements, updates, type) {
    ...
    Object.keys(updates).forEach((update) => {
        ...
        const isTransitive = topLevelDeps.indexOf((0, standardize_package_name_1.standardizePackageName)(pkgName)) < 0;
        if (type === 'transitive-pins' ? isTransitive : !isTransitive) {
            const [name, newVersion] = upgradeTo.split('@');
            lowerCasedUpdates[update] = {
                ...updates[update],
                upgradeTo: `${(0, standardize_package_name_1.standardizePackageName)(name)}@${newVersion}`,
            };
        }
    });
...
function generatePins(requirements, updates, referenceFileInChanges) {
    ...
    const standardizedPins = (0, calculate_relevant_fixes_1.calculateRelevantFixes)(requirements, updates, 'transitive-pins');
    ...
    const pinnedRequirements = Object.keys(standardizedPins)
        ...
        const newRequirement = `${(0, standardize_package_name_1.standardizePackageName)(pkgName)}>=${newVersion}`;
        changes.push({
            from: `${pkgName}@${version}`,
            to: `${pkgName}@${newVersion}`,
            issueIds: standardizedPins[pkgNameAtVersion].vulns,
            success: true,
            userMessage: `Pinned ${(0, standardize_package_name_1.standardizePackageName)(pkgName)} from ${version} to ${newVersion}${referenceFileInChanges ? ` (pinned in ${referenceFileInChanges})` : ''}`,
        });
...
function generateUpgrades(requirements, updates, referenceFileInChanges) {
    ...
    // we control this input internally, but its a low cost guard rail. Outputs a
    // mapping of upgrade to -> from, instead of the nested upgradeTo object.
    const normalizedUpgrades = (0, calculate_relevant_fixes_1.calculateRelevantFixes)(requirements, updates, 'direct-upgrades');
    ...
    requirements.map(({ name, originalName, versionComparator, version, originalText, extras, }) => {
        ...
        const upgrade = Object.keys(normalizedUpgrades).filter((packageVersionUpgrade) => {
            ...
            return `${(0, standardize_package_name_1.standardizePackageName)(pkgName)}@${versionAndMore}`.startsWith(`${(0, standardize_package_name_1.standardizePackageName)(name)}@${version}`);
        })[0];
        if (!upgrade) {
            return;
        }
...
function updateDependencies(parsedRequirementsData, updates, directUpgradesOnly = false, referenceFileInChanges) {
    ...
    const { updatedRequirements, changes: upgradedChanges } = (0, generate_upgrades_1.generateUpgrades)(requirements, updates, referenceFileInChanges);
    debug('Finished generating upgrades to apply');
    let pinnedRequirements = [];
    let pinChanges = [];
    if (!directUpgradesOnly) {
        ({ pinnedRequirements, changes: pinChanges } = (0, generate_pins_1.generatePins)(requirements, updates, referenceFileInChanges));
        debug('Finished generating pins to apply');
    }
    let updatedManifest = [
        ...(0, apply_upgrades_1.applyUpgrades)(requirements, updatedRequirements),
        ...pinnedRequirements,
    ].join('\n');
...
function extractDependencyDataFromLine(requirementText, line) {
    try {
        ...
        if (result !== null) {
            requirement.name = (0, standardize_package_name_1.standardizePackageName)(result[1]);
...
async function pipenvPipfile(fixable, options) {
    ...
    await (0, package_tool_supported_1.checkPackageToolSupported)('pipenv', options);
    for (const [index, entity] of fixable.entries()) {
        ...
        spinner.start();
        const { failed, succeeded, skipped } = await (0, update_dependencies_1.updateDependencies)(entity, options);
        handlerResult.succeeded.push(...succeeded);
        handlerResult.failed.push(...failed);
        handlerResult.skipped.push(...skipped);
        spinner.stop();
...
function generateUpgrades(entity) {
    const { remediation } = (0, validate_required_data_1.validateRequiredData)(entity);
    ...
    for (const pkgAtVersion of Object.keys(pins)) {
        ...
        upgrades.push(`${(0, standardize_package_name_1.standardizePackageName)(pkgName)}==${newVersion}`);
    }
    return { upgrades };
}
exports.generateUpgrades = generateUpgrades;
...
async function fixAll(entity, options) {
    ...
    const changes = [];
    try {
        const { upgrades } = await (0, generate_upgrades_1.generateUpgrades)(entity);
        if (!upgrades.length) {
        ...
        if (upgrades.length) {
            changes.push(...(await (0, pipenv_add_1.pipenvAdd)(entity, options, upgrades)));
        }
        if (!changes.length) {
            throw new no_fixes_applied_1.NoFixesCouldBeAppliedError();
        }
        if (!changes.some((c) => (0, attempted_changes_summary_1.isSuccessfulChange)(c))) {
    ...
    try {
        ...
        if (upgrades.length) {
            for (const upgrade of upgrades) {
                changes.push(...(await (0, pipenv_add_1.pipenvAdd)(entity, options, [upgrade])));
            }
        }
        if (!changes.length) {
            throw new no_fixes_applied_1.NoFixesCouldBeAppliedError();
...
async function pipenvAdd(entity, options, upgrades) {
    const changes = [];
    let pipenvCommand;
    const { remediation, targetFile } = (0, validate_required_data_1.validateRequiredData)(entity);
    try {
        ...
        changes.push(...(0, attempted_changes_summary_1.generateSuccessfulChanges)(upgrades, remediation.pin));
    }
    catch (error) {
        changes.push(...(0, attempted_changes_summary_1.generateFailedChanges)(upgrades, remediation.pin, error, pipenvCommand));
    }
    return changes;
}
...
async function poetry(fixable, options) {
    ...
    await (0, package_tool_supported_1.checkPackageToolSupported)('poetry', options);
    for (const [index, entity] of fixable.entries()) {
        const spinner = ora({ isSilent: options.quiet, stream: process.stdout });
...
async function generateUpgrades(entity) {
    ...
    const prodTopLevelDeps = Object.keys((_a = pyProjectToml.tool.poetry.dependencies) !== null && _a !== void 0 ? _a : {}).map((dep) => (0, standardize_package_name_1.standardizePackageName)(dep));
    const devTopLevelDeps = Object.keys((_b = pyProjectToml.tool.poetry['dev-dependencies']) !== null && _b !== void 0 ? _b : {}).map((dep) => (0, standardize_package_name_1.standardizePackageName)(dep));
    const upgrades = [];
    const devUpgrades = [];
    for (const pkgAtVersion of Object.keys(pins)) {
        const pin = pins[pkgAtVersion];
        const newVersion = pin.upgradeTo.split('@')[1];
        const [pkgName] = pkgAtVersion.split('@');
        const upgrade = `${(0, standardize_package_name_1.standardizePackageName)(pkgName)}==${newVersion}`;
...
exports.updateDependencies = void 0;
const debugLib = __webpack_require__(15158);
const generate_upgrades_1 = __webpack_require__(52307);
...
async function fixAll(entity, options) {
    ...
    const { upgrades, devUpgrades } = await (0, generate_upgrades_1.generateUpgrades)(entity);
    // TODO: for better support we need to:
    // 1. parse the manifest and extract original requirements, version spec etc
    // 2. swap out only the version and retain original spec
    // 3. re-lock the lockfile
    ...
    try {
        ...
        // update prod dependencies first
        if (upgrades.length) {
            changes.push(...(await (0, poetry_add_1.poetryAdd)(entity, options, upgrades)));
        }
        // update dev dependencies second
        if (devUpgrades.length) {
            const installDev = true;
            changes.push(...(await (0, poetry_add_1.poetryAdd)(entity, options, devUpgrades, installDev)));
        }
        if (!changes.length) {
            throw new no_fixes_applied_1.NoFixesCouldBeAppliedError();
        }
...
async function fixSequentially(entity, options) {
    ...
    try {
        ...
        if (upgrades.length) {
            for (const upgrade of upgrades) {
                changes.push(...(await (0, poetry_add_1.poetryAdd)(entity, options, [upgrade])));
            }
        }
        // update dev dependencies second
        if (devUpgrades.length) {
            for (const upgrade of devUpgrades) {
                const installDev = true;
                changes.push(...(await (0, poetry_add_1.poetryAdd)(entity, options, [upgrade], installDev)));
            }
        }
        if (!changes.length) {
            throw new no_fixes_applied_1.NoFixesCouldBeAppliedError();
...
async function poetryAdd(entity, options, upgrades, dev) {
    ...
    try {
        ...
        changes.push(...(0, attempted_changes_summary_1.generateFailedChanges)(upgrades, remediation.pin, error, poetryCommand));
    }
    return changes;
}
...
async function pythonFix(entities, options) {
    ...
    const { entitiesPerType, skipped: notSupported } = (0, map_entities_per_handler_type_1.mapEntitiesPerHandlerType)(entities);
    ...
    await pMap(Object.keys(entitiesPerType), async (projectType) => {
        ...
        spinner.render();
        try {
            const handler = (0, load_handler_1.loadHandler)(projectType);
            // drop unsupported Python entities early so only potentially fixable items get
            // attempted to be fixed
            const { fixable, skipped: notFixable } = await (0, is_supported_1.partitionByFixable)(projectsToFix);
...
function mapEntitiesPerHandlerType(entities) {
    ...
    const skipped = [];
    for (const entity of entities) {
        const type = (0, get_handler_type_1.getHandlerType)(entity);
        if (type) {
            entitiesPerType[type].push(entity);
            continue;
        }
